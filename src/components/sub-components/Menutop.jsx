...
// import { TransitionGroup, CSSTransition } from "react-transition-group";
// import "../../css/app.css";
// import { bakery, food, beverages, onWheels } from "../../menu-data"; // Import the "onWheels" data
// import MySVGComponent from "./MySVGComponent";
...
//         setIndexToShow(food.findIndex((item) => item.name === subCat));
//       }
//       // Add a check for "onWheels" category here if needed
//       if (cat === "onWheels") {
//         setIndexToShow(onWheels.findIndex((item) => item.name === subCat));
//       }
//     }
...
//     // Add a check for "onWheels" category here if needed
//     if (cat === "onWheels") {
//       setSelectedTab("onWheels");
//       setFilteredItems(onWheels);
//     }
//   }, []);
...
//         break;
//       case "onWheels":
//         setFilteredItems(onWheels); // Update filtered items for "On Wheels"
//         break;
//       default:
//         setFilteredItems([]);
//     }
...
//     window.onload = () => {
//       if (itemNameRef.current) {
//         const itemToScrollTo = document.querySelector(
//           `[data-item-name="${itemNameRef.current}"]`
//         );
//         if (itemToScrollTo) {
//           console.log(itemToScrollTo);
//           itemToScrollTo.scrollIntoView({ behavior: "smooth", block: "center" });
...
//                 </li>
//                 <li
//                   className={selectedTab === "onWheels" ? "filter-active" : ""} // Add "onWheels" tab
//                   onClick={() => handleTabClick("onWheels")} // Handle "onWheels" tab click
//                 >
//                   On Wheels
//                 </li>
//               </ul>
...
import { TransitionGroup, CSSTransition } from "react-transition-group";
import "../../css/app.css";
import { bakery, food, beverages, onWheels } from "../../menu-data"; // Import the menu data
import MySVGComponent from "./MySVGComponent";
...
const Menutop = () => {
  ...
  useEffect(() => {
    ...
    if (cat === "bakery") {
      setSelectedTab("bakery");
      setFilteredItems(sortByPrice(bakery)); // Sort "bakery" data by price
    }

    if (cat === "beverages") {
      setSelectedTab("beverages");
      setFilteredItems(sortByPrice(beverages)); // Sort "beverages" data by price
    }

    if (cat === "food") {
      setSelectedTab("food");
      setFilteredItems(sortByPrice(food)); // Sort "food" data by price
    }

    // Add a check for "onWheels" category here if needed
    if (cat === "onWheels") {
      setSelectedTab("onWheels");
      setFilteredItems(sortByPrice(onWheels)); // Sort "onWheels" data by price
    }
  }, []);
                    ...
                    >
                      <div className="accordion-item">
                        ...
                        >
                          ...
                          >
                           {category.data &&
                            sortByPrice(category.data).map((item, index) => (
                              <div
                                className="menu-item"
                                key={item.uuid}
                                data-item-name={item.name}
                              >
                                <img
                                  ...
                                  src={item.image}
                                  alt=""
                                  style={{ marginTop: item.description ? '-6px' : '-20px' }}
                                />
                                <div className="menu-content">
                                  <a href="#">{item.name}</a>
                                  <span>₹{item.price}</span>
